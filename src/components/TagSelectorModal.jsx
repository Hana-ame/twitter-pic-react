...
const INITIAL_FIXED_TAGS = ["男娘", "女装", "COS", "Lolita", "露出", "白幼瘦", "白虎", "大奶", "贫乳"];
const LOCAL_STORAGE_KEY = "user_custom_tags";

const TagSelectorModal = ({ isOpen, onClose, onConfirm, username, initialValues = {} }) => {
    ...
    useEffect(() => {
        if (isOpen) {
            ...
            try {
                const storedTags = JSON.parse(localStorage.getItem(LOCAL_STORAGE_KEY) || "[]");
                // 合并固定tag和存储的tag，去重
                const mergedTags = Array.from(new Set([...INITIAL_FIXED_TAGS, ...storedTags]));
    ...
    const getTagClass = (tag) => {
        ...
        const baseClass = "px-3 py-1 rounded-md text-sm border transition-colors duration-200 cursor-pointer select-none";
        
        if (score === 1) return `${baseClass} bg-blue-100 text-blue-700 border-blue-300 hover:bg-blue-200`;
        if (score === -1) return `${baseClass} bg-red-100 text-red-700 border-red-300 hover:bg-red-200`;
    ...
    const handleAddCustomTag = () => {
        ...
        if (!displayOtherTags.includes(newTag)) {
            ...
            const customOnly = newTagsList.filter(t => !INITIAL_FIXED_TAGS.includes(t));
                ...
                <div className="p-5 border-b border-gray-100 flex justify-between items-center">
                    <div>
                        <h2 className="text-lg font-bold text-gray-800">
                            {initialValues && Object.keys(initialValues).length > 0 ? "修改标签" : "添加标签"} 
                ...
                <div className="p-6 space-y-6 flex-1 overflow-y-auto">
                    {PRESET_CATEGORIES.map((category) => (
                        <div key={category.title}>
                            <h3 className="text-xs font-bold text-gray-400 mb-2 uppercase tracking-wider">{category.title}</h3>
                            <div className="flex flex-wrap gap-2">
                                {category.tags.map((tag) => (
                                    <button key={tag} onClick={() => handleTagClick(tag)} className={getTagClass(tag)}>
                                        {tag}
                                    </button>
                                ))}
                            </div>
                    ...
                    <div>
                        <h3 className="text-xs font-bold text-gray-400 mb-2 uppercase tracking-wider">其他 / 自定义</h3>
