...
import { useGayMode, useGayTags, isGayTag, matchesGayMode, hasGayTag } from "../utils/gayMode";
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (flag) {
        ...
        // Gay 模式正好取反：
        // 开启 Gay 模式时：只显示包含 Gay 标签的内容（未包含则隐藏）
        // 关闭 Gay 模式时：隐藏包含 Gay 标签的内容（包含则隐藏）
        const hasGay = hasGayTag(user.tags, gayTags) || hasGayTag(userTagKeys, gayTags);
        const matchesGay = gayMode ? hasGay : !hasGay;
        if (!matchesGay) {
          setBlocked(true);
          flag = false;
        }
        ...
        if (flag && userTagKeys.length > 0) {
          ...
          if (rawRules) {
            try {
              ...
              if (Array.isArray(rules.block)) {
                blockRules = rules.block.filter((t) => !isGayTag(t, gayTags));
                if (blockRules.length === 0) blockRules = ["无关内容"];
              }
