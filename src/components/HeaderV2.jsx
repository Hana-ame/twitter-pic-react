...
const HeaderV2 = ({ user, onClick }) => {
    ...
    function fetchAndSet() {
        ...
        getMetaData(user.username, user.last_modify).then(data => {
    ...
    useEffect(() => {
        ...
        try {
            ...
            if (s) {
                ...
                if (o[user.username] === true) { // 或者 if (Boolean(o[username])) 用于更宽松的判断
                    setBlocked(true);
                    flag = false;
                }
            }
            // TBD：这里需要再添加一段逻辑，也即筛选tags
            const tags = user.tags;
            const tagRules = window.localStorage.getItem("tag-rules");
            // TBD：逻辑
            // 
        } catch (e) {
            console.error(e);
